...
export default function LiveControlPanel({ presentationId }: { presentationId: string }) {
    ...
    const loadLyrics = async () => {
      ...
      try {
        ...
        if (lyricsText) {
          ...
          const parsedSlides = lines.map((content, index) => ({
            id: `slide-${index}`,
            content,
  ...
  const nextSlide = () => {
    if (currentSlideIndex < slides.length - 1) {
      ...
      if (window.api) {
        window.api.sendControl('slide', { index: newIndex });
      }
                ...
                {slides.map((slide, index) => (
                  <motion.button
                    ...
                    onClick={() => {
                      ...
                      if (window.api) {
                        window.api.sendControl('slide', { index });
                      }
      ...
      <div className="flex-shrink-0 h-20 border-t border-purple-500/20 bg-black/60 backdrop-blur-md">
        <div className="h-full flex items-center justify-center gap-3 px-6">
          ...
          <Button
            ...
            onClick={async () => {
              if (window.api) {
                const result = await window.api.openBackgroundDialog();
                if (result) {
                  window.api.sendControl('background', result);
          ...
          <Button
            ...
            onClick={async () => {
              if (window.api) {
                const result = await window.api.openThemeDialog();
                if (result) {
                  window.api.sendControl('theme', result);
          ...
          <Button
            ...
            onClick={async () => {
              if (window.api) {
                const result = await window.api.openTransitionDialog();
                if (result) {
                  window.api.sendControl('transition', result);
