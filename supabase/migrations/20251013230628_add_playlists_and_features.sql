...
-- SONGS TABLE UPDATES
-- ============================================================================
-- A tabela songs já existe, então vamos adicionar apenas as colunas que faltam

-- Adicionar novas colunas se não existirem
ALTER TABLE songs ADD COLUMN IF NOT EXISTS url TEXT;
ALTER TABLE songs ADD COLUMN IF NOT EXISTS lyrics_preview TEXT;
ALTER TABLE songs ADD COLUMN IF NOT EXISTS lyrics_length INTEGER;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'songs_artist_title_key'
  ) THEN
    ALTER TABLE songs ADD CONSTRAINT songs_artist_title_key UNIQUE(artist, title);
  END IF;
END $$;

-- Índices para performance (alguns já podem existir)
...
-- TAGS PARA PLAYLISTS
-- ============================================================================
-- Nota: Esta tabela depende da tabela 'tags' que deve ter sido criada em migrações anteriores

-- Apenas criar a tabela se a tabela tags existir
DO $$
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Tabela tags não existe. Pulando criação de playlist_tags.';
  END IF;
END $$;
...
-- SONG ANALYTICS
-- ============================================================================
-- Nota: Esta tabela depende da tabela 'presentations' que deve ter sido criada em migrações anteriores

-- Criar tabela de uso de músicas
DO $$
BEGIN
  ...
  ELSE
    -- Criar sem foreign key para presentations se não existir
    CREATE TABLE IF NOT EXISTS song_usage (
      id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
      song_id UUID REFERENCES songs(id) ON DELETE CASCADE,
    ...
    RAISE NOTICE 'Tabela presentations não existe. Criando song_usage sem foreign key.';
  END IF;
END $$;
...
-- ============================================================================
-- Nota: Esta view depende da tabela 'presentation_history' que deve ter sido criada em migrações anteriores

-- Criar view apenas se a tabela presentation_history existir
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'presentation_history') THEN
    EXECUTE '
    ...
    ';
  ELSE
    RAISE NOTICE 'Tabela presentation_history não existe. Pulando criação de presentation_stats view.';
  END IF;
END $$;
